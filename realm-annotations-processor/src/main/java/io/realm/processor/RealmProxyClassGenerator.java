...
public class RealmProxyClassGenerator {
    ...
    public void generate() throws IOException, UnsupportedOperationException {
        ...
        imports.add("io.realm.dynamic.RealmType");
    ...
    private void emitInitTableMethod(JavaWriter writer) throws IOException {
        ...
        for (VariableElement field : metadata.getFields()) {
            ...
            if (Constants.JAVA_TO_REALM_TYPES.containsKey(fieldTypeCanonicalName)) {
                ...
                writer.endControlFlow();
                writer.emitStatement("table.addColumnLink(RealmType.LINK, \"%s\", transaction.getTable(\"%s%s\"))",
                ...
                writer.endControlFlow();
                writer.emitStatement("table.addColumnLink(RealmType.LINK_LIST, \"%s\", transaction.getTable(\"%s%s\"))",
    ...
    private void emitValidateTableMethod(JavaWriter writer) throws IOException {
        ...
        writer.emitStatement("Map<String, RealmType> columnTypes = new HashMap<String, RealmType>()");
        ...
        for (VariableElement field : metadata.getFields()) {
            ...
            if (Constants.JAVA_TO_REALM_TYPES.containsKey(fieldTypeCanonicalName)) {
                ...
                writer.emitStatement("throw new RealmMigrationNeededException(transaction.getPath(), \"Missing field '%s'\")", fieldName);
                writer.endControlFlow();
                writer.beginControlFlow("if (columnTypes.get(\"%s\") != RealmType.LINK)", fieldName);
                writer.emitStatement("throw new RealmMigrationNeededException(transaction.getPath(), \"Invalid type '%s' for field '%s'\")",
                        fieldTypeSimpleName, fieldName);
                writer.endControlFlow();
                writer.beginControlFlow("if (!transaction.hasTable(\"%s%s\"))", Constants.TABLE_PREFIX, fieldTypeSimpleName);
                ...
                writer.emitStatement("throw new RealmMigrationNeededException(transaction.getPath(), \"Missing field '%s'\")", fieldName);
                writer.endControlFlow();
                writer.beginControlFlow("if (columnTypes.get(\"%s\") != RealmType.LINK_LIST)", fieldName);
                writer.emitStatement("throw new RealmMigrationNeededException(transaction.getPath(), \"Invalid type '%s' for field '%s'\")",
